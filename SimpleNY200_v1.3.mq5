//+------------------------------------------------------------------+
//| SimpleNY200 v1.3 - New York Opening Range Breakout Strategy     |
...
//| v1.3: Advanced Risk Management - CMyToolkit Integration          |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, Bittrader Development Team"
#property link      "https://www.youtube.com/@bittrader9259"
#property version   "1.30"
#property description "SimpleNY200 v1.3 - Advanced Risk Management for NAS100"
#property strict
...
//+------------------------------------------------------------------+
//| SECTION 2B: CMyToolkit CLASS (v1.3 - Copied from Indicator)     |
//+------------------------------------------------------------------+
class CMyToolkit {
...
input string          EAIdentifier        = "SimpleNY200_v1.3";  // Identificador del EA
...
//+------------------------------------------------------------------+
input group "==== Gesti贸n de Riesgo Avanzada (v1.3) ===="
...
//+------------------------------------------------------------------+
//| SECTION 10B: INPUT PARAMETERS - ATR STOP LOSS (v1.3)            |
//+------------------------------------------------------------------+
input group "==== Stop Loss Din谩mico con ATR (v1.3) ===="
...
int OnInit()
{
    ...
    if(EnableFileLogging) {
        ...
        if(g_FileHandle != INVALID_HANDLE) {
            ...
            FileWrite(g_FileHandle, "鈺�   SimpleNY200 v1.3 - Session Log [ADVANCED RISK MGMT]    鈺�");
    ...
    // STEP 8: LOG CONFIGURATION (v1.3)
    LogMessage("=== CONFIGURACION COMPLETA (v1.3) ===", LOG_INFO);
    ...
    // v1.3 NEW: Advanced Risk Management parameters
    ...
    LogMessage("鉁� SimpleNY200 v1.3 inicializado correctamente [ADVANCED RISK MANAGEMENT ACTIVE]", LOG_INFO);
...
{
    LogMessage("SimpleNY200 v1.3 deinicializando - Raz贸n: " + IntegerToString(reason), LOG_INFO);
    ...
    Print("SimpleNY200 v1.3 deinicializado exitosamente [ADVANCED RISK MANAGEMENT]");
}
...
void OnTick()
{
    ...
    // v1.2 FIX: Detectar cambio de d铆a en el PRIMER TICK del nuevo d铆a,
    ...
    // Detectar cambio de d铆a
    if(dt.day != lastDay && lastDay != 0) {
        ResetDailyData();
        LogToFile("鉁� Reset diario ejecutado - D铆a: " + IntegerToString(dt.day), LOG_INFO);
    }
    lastDay = dt.day;
...
//+------------------------------------------------------------------+
//| Calculate position size based on risk (v1.3 - UPGRADED)         |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Validate trade conditions before entry (v1.3 - UPGRADED)        |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Calculate Stop Loss (v1.3 - UPGRADED with ATR support)          |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Normalize lot size (v1.3 - UPGRADED with proper rounding)       |
//+------------------------------------------------------------------+
